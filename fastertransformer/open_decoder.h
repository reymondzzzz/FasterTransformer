...
namespace fastertransformer {
    ...
    public:
        ...
        void initialize(DecoderInitParam<DataType_> param, DataType_ *buf, void *cublas_workapsce,
            ...
            query_buf_ =
                    buf + buf_size; //store the query values (from_tensor * Q) in both masked and multi-head attention
            key_buf_ = query_buf_ + buf_size;
            value_buf_ = key_buf_ + buf_size;
            context_buf_ = value_buf_ +
                           buf_size; //store the context result (softmax(qk)v) in both masked and multi-head attention
            ...
            if (is_fuse_QKV_in_normal_gemm_ == false && is_fuse_QKV_in_batched_gemm_ == true) {
                ...
                cudaMemcpyAsync((void *) qkv_kernel_, hA, sizeof(DataType_ *) * 9, cudaMemcpyHostToDevice,
                                param_.stream);
            }
        }
        ...
        void forward(const DataType_ *from_tensor, const DataType_ *memory_tensor,
            ...
            try {
                ...
                masked_multi_head_attention(norm_from_tensor_buf_, key_cache_, value_cache_, masked_output_buf_,
                                            finished, step, decoder_max_seq_len);
                POP_RANGE
                ...
                if (is_cross_attention == true) {
                    ...
                    */
                    ...
                    check_cuda_error(cudaGetLastError());
#endif
                    ffn(norm_cross_output_buf_, ffn_inner_buf_, decoder_output, m,
                        4 * t_parallel_param_.local_hidden_units_, hidden_units_, ActivationType::RELU);
#ifndef NDEBUG
                    cudaDeviceSynchronize();
                    check_cuda_error(cudaGetLastError());
#endif
                    add_bias_input_kernelLauncher(decoder_output, param_.ffn.output_weight.bias, cross_output_buf_, m,
                                                  hidden_units_, param_.stream);
                } else {
                    add_bias_input_layernorm_2_kernelLauncher(from_tensor,
                                                              param_.ffn_layernorm.gamma,
                                                              param_.ffn_layernorm.beta,
                    ...
                    ffn(norm_masked_output_buf_, ffn_inner_buf_, decoder_output, m,
                        4 * t_parallel_param_.local_hidden_units_, hidden_units_, ActivationType::GELU);
                    POP_RANGE
                    ...
                    check_cuda_error(cudaGetLastError());
#endif
                    add_bias_input_kernelLauncher(decoder_output, param_.ffn.output_weight.bias, masked_output_buf_, m,
                                                  hidden_units_, param_.stream);
                }
#ifndef NDEBUG
                cudaDeviceSynchronize();
...
#endif
                PUSH_RANGE("Transformer/slf_attn")
                masked_multi_head_attention_v2(norm_from_tensor_buf_, key_cache_, value_cache_, masked_output_buf_,
                                               finished, step, decoder_max_seq_len, max_input_len, input_lengths);
                POP_RANGE
        ...
        size_t getContextWorkspaceSize(const int seq_len, const int local_batch_size) {
            ...
            const size_t qk_buf_size =
                    (size_t) (ceil(local_batch_size * t_parallel_param_.local_head_num_ * seq_len * seq_len / 4.)) * 4;
        ...
        void forward_context(DataType_ *workspace,
            ...
            try {
                ...
                const int qk_buf_size =
                        (int) (ceil(local_batch_size * t_parallel_param_.local_head_num_ * seq_len * seq_len / 4.)) * 4;
                ...
                // For GPT decoder
                PUSH_RANGE("Transformer/MLP");
                ffn(norm_masked_output_buf, ffn_inner_buf, decoder_output, m, 4 * t_parallel_param_.local_hidden_units_,
                    hidden_units_, ActivationType::GELU);
                POP_RANGE
#ifndef NDEBUG
                cudaDeviceSynchronize();
                check_cuda_error(cudaGetLastError());
#endif
                add_bias_input_kernelLauncher(decoder_output, param_.ffn.output_weight.bias, masked_output_buf, m,
                                              hidden_units_, param_.stream);
        ...
        void masked_multi_head_attention(const DataType_ *from_tensor, DataType_ *key_cache_,
            ...
            if (is_fuse_QKV_in_normal_gemm_ == true) {
                ...
                if (is_fuse_QKV_in_batched_gemm_ == true) {
                    cublasGemmAlgo_t cublasAlgo = static_cast<cublasGemmAlgo_t>(getAlgoIdFromMap(cublasAlgoMap_, 3, n,
                                                                                                 m, k,
                                                                                                 std::is_same<float, DataType_>::value
                                                                                                 ? FLOAT_DATATYPE
                                                                                                 : HALF_DATATYPE));
        ...
        void masked_multi_head_attention_v2(const DataType_ *from_tensor, DataType_ *key_cache_,
            ...
            fusedQKV_masked_attention_dispatch_v2<DataType_>(
                    ...
                    value_cache_,
                    context_buf_, finished, param_.request_batch_size, l_parallel_param_.local_batch_size,
                    t_parallel_param_.local_head_num_, size_per_head_, step, max_seq_len, max_input_len, input_lengths,
                    param_.stream);
        ...
        void ffn(const DataType_ *input, DataType_ *ffn_inner, DataType_ *output,
            ...
            add_bias_act_kernelLauncher(ffn_inner, param_.ffn.intermediate_weight.bias, m1, inner_size, activation_type,
                                        param_.stream);
        ...
        void unfused_masked_multi_head_attention(DataType_ *workspace,
            ...
            cublasGemmAlgo_t cublasAlgo = static_cast<cublasGemmAlgo_t>(getAlgoIdFromMap(cublasAlgoMap_,
                                                                                         local_batch_size *
                                                                                         t_parallel_param_.local_head_num_,
                                                                                         seq_len, seq_len,
                                                                                         size_per_head_,
            ...
            cublasAlgo = static_cast<cublasGemmAlgo_t>(getAlgoIdFromMap(cublasAlgoMap_, local_batch_size *
                                                                                        t_parallel_param_.local_head_num_,
                                                                        size_per_head_, seq_len, seq_len,
                                                                        std::is_same<float, DataType_>::value
                                                                        ? FLOAT_DATATYPE : HALF_DATATYPE));
            ...
            check_cuda_error(cublasGemmStridedBatchedEx(param_.cublas_handle,
                                                        ...
                                                        &beta,
                                                        attn_trans_out, CType_, size_per_head_,
